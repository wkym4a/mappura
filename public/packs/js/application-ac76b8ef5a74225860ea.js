...
function (_Controller) {
  ...
  _createClass(_default, [{
    ...
    value: function initialize() {
      ...
      this.map.drawMap(new Y.LatLng(35.66572, 139.73100), 17, Y.LayerSetId.NORMAL); // var center = new Y.CenterMarkControl;

      var control = new Y.LayerSetControl();
      var sliderzoom = new Y.SliderZoomControlVertical(); // var searchcontrol = new Y.SearchControl();
      // this.map.addControl(center);

      this.map.addControl(control);
      this.map.addControl(sliderzoom); // this.map.addControl(searchcontrol);
      //「プラン」由来のピンを作成

      this.make_pins(this.pins_belongs_to_planTarget, "plan", markers_belongs_to_plan); //「ワークボックス」由来のピンを作成
        ...
        for (var i = 0; i < this.pins.length; i++) {
          ...
          switch (pin_type) {
            case "plan":
              //クリックすると詳細を表示(詳細の中身はこの段階では未作成、
              // 「make-speech-bubble_controller.js」のロードイベントで描写する)
              marker.bindInfoWindow('<div data-controller="make-speech-bubble-plan-member" class="speech_bubble_box">');
              break;

            case "workbox":
              //クリックすると詳細を表示(詳細の中身はこの段階では未作成、
              // 「make-speech-bubble_controller.js」のロードイベントで描写する)
              marker.bindInfoWindow('<div data-controller="make-speech-bubble-plan-candidate" class="speech_bubble_box">');
              break;
          } // //クリックすると詳細を表示(詳細の中身はこの段階では未作成、
          // // 「make-speech-bubble_controller.js」のロードイベントで描写する)
          // // 描写もまた後で
...
_default.targets = ["map", "plan_id", "pins_belongs_to_plan", "pins_belongs_to_workbox"];
...
/******/ });
//# sourceMappingURL=application-ac76b8ef5a74225860ea.js.map