...
function (_Controller) {
  ...
  _createClass(_default, [{
    ...
    value: function initialize(e) {
      ...
      this.latitudeTarget.disabled = false;
      this.longitudeTarget.disabled = false;
    ...
    value: function set_address_by_latlng() {
      ...
      if (isNaN(this.latitudeTarget.value) == false || isNaN(this.longitudeTarget.value) == false) {
        txt_msg.text = "経度、緯度には数値を入力してください。";
        return;
      }

      if (this.latitudeTarget.value < -180 || this.latitudeTarget.value > 180 || this.longitudeTarget.value < -90 || this.longitudeTarget.value > 90) {
        txt_msg.text = '存在しない座標です。\n緯度は「-180」〜「180」、経度は「-90」〜「90」の範囲で入力してください。';
        return;
      }
    ...
    get: function get() {
      ...
      // var latlng = new Y.LatLng(-48.61153073015411, 1600.3550844726565);
      // var request = { "latlng": latlng };
      //
      // var geocoder = new Y.GeoCoder();
      // geocoder.execute( request , function( ydf, status ) {
      //     if ( ydf.features.length > 0 ) {
    ...
    get: function get() {
      ...
      if (latlng_decided_flg == false) {
        latlng_decided_flg = true;
        this.latitudeTarget.disabled = true;
        this.longitudeTarget.disabled = true;
...
/******/ });
//# sourceMappingURL=application-03c1e841e0d0c24cd8a9.js.map