    ...
    get: function get() {
      ...
      $.ajax({
        ...
        if (data.length == 0) {
          ...
          switch (type) {
            case "workbox":
              form.action = "/plans/" + selected_id + "/workbox_pins";
              method.value = "post";
              break;
          ...
          switch (type) {
            case "workbox":
              form.action = "/plans/" + selected_id + "/workbox_pins/" + data[0].id;
              method.value = "delete";
              break;
...
function (_Controller) {
  ...
  return _default;
}(stimulus__WEBPACK_IMPORTED_MODULE_0__["Controller"]); // type: 'GET',
// url: '/plans/' + plan_id + '/plan_pins/new',
...
function (_Controller) {
  ...
  _createClass(_default, [{
    ...
    value: function move(el) {
      ...
      if (this.action_pin_id == "") return; // 座標（＝緯度経度）を取得……↑で取得した「this.index_num」に該当するターゲットがtextcontentに持つ値をハッシュで獲得
...
/******/ });
//# sourceMappingURL=application-6c893893e30672e4b039.js.map